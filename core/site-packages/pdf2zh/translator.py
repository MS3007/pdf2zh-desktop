...
class DeepseekTranslator(OpenAITranslator):
    ...
    # DeepSeek 的 system prompt：严格约束只输出译文
    _SYSTEM_PROMPT = (
        "You are a machine translation engine. "
        "STRICT RULES: "
        "1) Output ONLY the translated text. "
        "2) NEVER add notes, annotations, comments, or explanations. "
        "3) NEVER add parenthetical remarks like (注：...) or (Note:...). "
        "4) NEVER explain your translation choices. "
        "5) Keep formula notation {v*} or {{v*}} unchanged. "
        "6) Preserve the original text structure and length."
    )
